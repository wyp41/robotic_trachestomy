...
namespace MixedReality.Toolkit.SpatialManipulation
{
    ...
    public class ObjectManipulator : StatefulInteractable
    {
        ...
        public Vector3 updateTranslate;
        public Quaternion updateRotation;
        public int is_selected = 0;
        public Vector3 objectLocalAttachPoint;
        public Vector3 reject_rotation;
        ...
        protected override void OnSelectEntered(SelectEnterEventArgs args)
        {
            ...
            using (OnSelectEnteredPerfMarker.Auto())
            {
                ...
                var attachCentroid = sumPos / Mathf.Max(1, count);
                objectLocalAttachPoint = Quaternion.Inverse(targetTransform.Rotation) * (attachCentroid - targetTransform.Position);
                objectLocalAttachPoint = objectLocalAttachPoint.Div(targetTransform.Scale);
                startInputRotation = interactorsSelecting[0].GetAttachTransform(this).rotation;
                ...
                if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Dynamic)
                {
                    ...
                    using (RotateLogicMarker.Auto())
                    {
                        if (allowedManipulations.IsMaskSet(TransformFlags.Rotate))
                        {
                            ...
                            updateRotation = interactorsSelecting[0].GetAttachTransform(this).rotation * Quaternion.Inverse(startInputRotation);
                        }
                    }
                    ...
                    using (MoveLogicMarker.Auto())
                    {
                        if (allowedManipulations.IsMaskSet(TransformFlags.Move))
                        {
                            ...
                            var attachCentroid = sumPos / Mathf.Max(1, count);
                            var scaledLocalAttach = Vector3.Scale(objectLocalAttachPoint, targetTransform.Scale);
                            var worldAttachPoint = targetTransform.Rotation * scaledLocalAttach + targetTransform.Position;
                            var trans = attachCentroid - worldAttachPoint;
                            //updateTranslate = Quaternion.Inverse(targetTransform.Rotation) * trans;
                            updateTranslate = trans;
                        }
                    }
        ...
        private void ApplyTargetTransform()
        {
            ...
            if (rigidBody == null)
            {
                var temp_Rotation = HostTransform.localRotation.eulerAngles;
                HostTransform.SetPositionAndRotation(targetTransform.Position, targetTransform.Rotation);
                var cur_Rotation = HostTransform.localRotation.eulerAngles;

                for (int i = 0; i < 3; i++)
                {
                    if (reject_rotation[i] == 1)
                    {
                        cur_Rotation[i] = temp_Rotation[i];
                    }
                }
                //cur_Rotation.z = temp_Rotation.z;
                HostTransform.localRotation = Quaternion.Euler(cur_Rotation);
        ...
        protected virtual void ModifyTargetPose(ref MixedRealityTransform targetPose, ref TransformFlags modifiedTransformFlags)
        {
            ...
            //updateTransform = MoveLogicMarker.updateTransform;
