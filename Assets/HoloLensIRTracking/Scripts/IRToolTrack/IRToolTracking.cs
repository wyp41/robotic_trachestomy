...
public class IRToolTracking : MonoBehaviour
{
    ...
    private void SetReferenceWorldCoordinateSystem()
    {
        ...
        var origin_pose = UnityEngine.Pose.identity;
        origin_pose.position = new Vector3(0, 0, 0) - Quaternion.Inverse(origin.rotation) * origin.position;
        origin_pose.rotation = Quaternion.Inverse(origin.rotation);
        //print(origin_pose);
#if ENABLE_WINMD_SUPPORT
        // Get Unity Origin Coordinate
#if UNITY_2021_2_OR_NEWER
        //var unityWorldOrigin = Microsoft.Windows.Perception.Spatial.SpatialCoordinateSystem
        Windows.Perception.Spatial.SpatialCoordinateSystem unityWorldOrigin = Microsoft.MixedReality.OpenXR.PerceptionInterop.GetSceneCoordinateSystem(origin_pose) as Windows.Perception.Spatial.SpatialCoordinateSystem;
